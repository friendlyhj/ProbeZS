...
import youyihj.probezs.socket.rpc.RpcRequest;

/**
 * @author youyihj
 */
public class RpcBracketCheckSocketHandler extends SimpleChannelInboundHandler<RpcRequest> {
    private static final JsonParser parser = new JsonParser();

    @Override
    protected void channelRead0(ChannelHandlerContext ctx, RpcRequest msg) throws Exception {
        JsonArray params = msg.getParams();
        if (msg.getMethod().equals("query")) {
            BracketHandlerResult result = BracketHandlerCaller.INSTANCE.query(params.get(0).getAsString(), params.get(1).getAsBoolean());
            ctx.channel().writeAndFlush(msg.response(BracketCheckHandler.outputJson(result)));
        }
    }
